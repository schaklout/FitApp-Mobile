...
public class RutinasActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        linearRutinasDelDia = findViewById(R.id.linearRutinasActivas);
        tvNoRutinas = findViewById(R.id.tvNoRutinasActivas);

        // --- Título y Calendario ---
        TextView tvTitulo = findViewById(R.id.tvTituloRutinas);
        if (tvTitulo != null) {
            String hoy = obtenerDiaHoy();
            tvTitulo.setText("Rutinas - " + hoy);
            tvTitulo.setOnClickListener(v -> new CalendarRutinasDialog(RutinasActivity.this).mostrar());
        }

        Button btnCalendario = findViewById(R.id.btnCalendario);
        if (btnCalendario != null) {
            btnCalendario.setOnClickListener(v -> new CalendarRutinasDialog(RutinasActivity.this).mostrar());
        }
        ...
        // --- Launcher para refrescar al volver ---
    ...
    private void cargarRutinas() {
        int dow = Calendar.getInstance().get(Calendar.DAY_OF_WEEK);
        int dia = (dow + 5) % 7; // 0=Lun..6=Dom

        ApiService.getRutinasPorDia(this, dia, new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
                runOnUiThread(() ->
                        Toast.makeText(RutinasActivity.this, "Error al obtener rutinas", Toast.LENGTH_SHORT).show()
                );
            }
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                if (response.isSuccessful()) {
                    ...
                    runOnUiThread(() ->
                            Toast.makeText(RutinasActivity.this, "Error del servidor (" + response.code() + ")", Toast.LENGTH_SHORT).show()
                    );
                }
            }
        });
    ...
    private LinearLayout crearCardRutina(String nombre, String descripcion, String nivel, int rutinaUsuarioId,
        ...
        if (vecesPorSemana > 0) {
            TextView tvVeces = new TextView(this);
        ...
        btnEntrenar.setOnClickListener(v -> {
            Intent intent = new Intent(RutinasActivity.this, EntrenamientoActivity.class);
